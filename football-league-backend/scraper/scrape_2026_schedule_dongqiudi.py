...
def fetch_dongqiudi_schedule_api(season_id: str = None) -> List[Dict[str, Any]]:
    # ===================== 优化：使用带重试的请求函数 =====================
    print("📡  开始请求懂球帝赛程API...")
    if season_id:
        url = f"https://sport-data.dongqiudi.com/soccer/biz/data/schedule?season_id={season_id}&app=dqd&version=830&platform=miniprogram&language=zh-cn&app_type="
    else:
        url = DONGQIUDI_SCHEDULE_API
    response = requests_with_retry(url, headers=DONGQIUDI_HEADERS, timeout=60)
