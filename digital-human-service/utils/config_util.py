...
def load_config(force_reload=False):
    ...
    # 如果没有设置 PROJECT_ID，强制不使用配置中心
    if not CONFIG_SERVER.get('PROJECT_ID'):
        using_config_center = False

    # 缓存机制：配置中心模式下使用 PROJECT_ID 作为缓存键，本地模式下使用 None
    cache_key = env_project_id if using_config_center else None
    if (
        not force_reload
        and _last_loaded_config is not None
        and _last_loaded_project_id == cache_key
    ):
        # 使用缓存配置，不打印日志（避免刷屏）
    ...
    if using_config_center:
        ...
        if (
            ...
            # 优先使用根目录的配置文件，如果不存在则使用 cache_data 目录的配置文件
            if root_config_complete:
                system_conf_path = default_system_conf_path
                config_json_path = default_config_json_path
            else:
                # 检查 cache_data 目录是否有配置文件
                cache_ready = os.path.exists(cache_system_conf_path) and os.path.exists(cache_config_json_path)
                if cache_ready:
                    system_conf_path = cache_system_conf_path
                    config_json_path = cache_config_json_path
                    util.log(1, f"使用 cache_data 目录的配置文件: {system_conf_path}, {config_json_path}")
                else:
                    # 两个地方都没有配置文件，稍后会尝试从配置中心加载
                    system_conf_path = default_system_conf_path
                    config_json_path = default_config_json_path
    ...
    # 记录配置文件检测情况
    if sys_conf_exists and config_json_exists:
        util.log(1, f"检测到本地配置文件: {system_conf_path}, {config_json_path}")
    elif not using_config_center:
        util.log(2, f"未检测到本地配置文件: {system_conf_path} ({'存在' if sys_conf_exists else '不存在'}), {config_json_path} ({'存在' if config_json_exists else '不存在'})")
...
def save_config(config_data):
    ...
    try:
        ...
        # 强制写入通义千问API配置
        system_config.set('key', 'gpt_base_url', 'https://dashscope.aliyuncs.com/compatible-mode/v1')
        system_config.set('key', 'gpt_api_key', 'sk-c9b8659683a541bfaa8580448ca67766')
        system_config.set('key', 'embedding_api_model', 'text-embedding-v3')
        system_config.set('key', 'gpt_model_engine', 'qwen-plus')

        # 保存system.conf
        with open(system_conf_path, 'w', encoding='UTF-8') as f:
            system_config.write(f)
    except Exception as e:
        util.log(2, f"保存system.conf时出错: {str(e)}")
