...
export class LAppWavFileHandler {
  ...
  /**
   * Inject pre-parsed PCM data synchronously.
   * This eliminates the async loadWavFile() delay — PCM is ready immediately
   * for RMS computation in the same frame.
   */
  public startWithPcm(
    ...
    // Set wav info
    this._wavFileInfo._numberOfChannels = numChannels;
    this._wavFileInfo._samplingRate = sampleRate;
    this._wavFileInfo._samplesPerChannel = samplesPerChannel;
    this._wavFileInfo._bitsPerSample = 16;

    // Copy PCM data
    this._pcmData = new Array(numChannels);
    for (let ch = 0; ch < numChannels; ch++) {
      this._pcmData[ch] = new Float32Array(pcmChannels[ch]);
    }
  }

  public start(data: string | ArrayBuffer): void {
    // Legacy async path — use startWithPcm() instead for sync lip-sync
    this._sampleOffset = 0;
    this._userTimeSeconds = 0.0;
    this._lastRms = 0.0;
