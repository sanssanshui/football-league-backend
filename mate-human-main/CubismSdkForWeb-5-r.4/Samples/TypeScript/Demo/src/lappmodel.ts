...
export class LAppModel extends CubismUserModel {
  ...
  public setRandomExpression(): void {
    if (this._expressions.getSize() == 0) {
      console.log('[LAppModel] 模型无表情文件，跳过随机表情');
      return;
    }
    ...
    this._fayClient.onMessage((message) => {
      ...
      if (message.Data && message.Data.Lips) {
        ...
        if (message.Data.IsEnd === 1) {
          ...
          // 对话结束后恢复默认表情（仅当模型有表情时）
          if (this._modelSetting.getExpressionCount() > 0) {
            console.log('[LAppModel] 😊 对话结束，恢复默认表情 F01');
  ...
  private setExpressionBySentiment(sentiment: number): void {
    // Hiyori模型没有表情文件，跳过表情设置
    if (this._modelSetting.getExpressionCount() === 0) {
      console.log(`[LAppModel] 情感值 ${sentiment} → 模型无表情文件，跳过`);
      return;
    }
   ...
   * @param sentiment 情感值：-2(非常消极) ~ +2(非常积极)
   *
   * Hiyori模型适配：TapBody组只有1个动作(m04)，优先使用Idle组动作
   */
  private setMotionBySentiment(sentiment: number): void {
    const tapMotionCount = this._modelSetting.getMotionCount(LAppDefine.MotionGroupTapBody);
    const idleMotionCount = this._modelSetting.getMotionCount(LAppDefine.MotionGroupIdle);

    // 优先使用TapBody动作，如果没有则使用Idle动作
    const useIdleAsFallback = tapMotionCount <= 1;
    const group = useIdleAsFallback ? LAppDefine.MotionGroupIdle : LAppDefine.MotionGroupTapBody;
    const motionCount = useIdleAsFallback ? idleMotionCount : tapMotionCount;

    if (motionCount === 0) {
      console.log(`[LAppModel] 情感值 ${sentiment} → 无可用动作`);
      return;
    }
    ...
    if (sentiment >= 0.8) {
      ...
      motionDesc = '消极';
      motionNo = Math.min(1, motionCount - 1); // 使用第2个动作
    } else if (Math.abs(sentiment) < 0.3) {
      motionDesc = '中性';
      motionNo = null; // 保持Idle状态
    } else {
      motionDesc = '轻微情感';
      motionNo = Math.floor(Math.random() * Math.min(3, motionCount));
    }

    if (motionNo !== null && motionNo < motionCount) {
      console.log(`[LAppModel] 情感值 ${sentiment} → [${group}] m${motionNo < 10 ? '0' + motionNo : motionNo}/${motionCount} (${motionDesc})`);
      this.startMotion(group, motionNo, LAppDefine.PriorityForce);
    } else {
      console.log(`[LAppModel] 情感值 ${sentiment} → ${motionDesc}，保持Idle`);
    }
  }
