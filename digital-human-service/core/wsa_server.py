...
class MyServer:
    ...
    async def __handler(self, websocket):
        ...
        consumer_task = asyncio.create_task(self.__consumer_handler(websocket))#接收
        producer_task = asyncio.create_task(self.__producer_handler(websocket))#发送
    ...
    async def __run_server(self):
        ...
        async with websockets.serve(self.__handler, self.__host, self.__port) as server:
    ...
    def __connect(self):
        ...
        if self.__server:
            ...
            return
        try:
            self.__event_loop.run_until_complete(self.__run_server())
        except Exception as e:
            util.log(1, f'WebSocket server error: {e}')
        finally:
...
class NoopServer:
    """No-op server stub for headless mode when WS servers aren't running."""
    def is_connected(self, username=None):
        return False
    def add_cmd(self, content):
        pass
    def get_client_output(self, username=None):
        return False
    def start_server(self):
        pass
...
# Noop fallback instances for headless mode
__noop_instance = NoopServer()
__noop_web_instance = NoopServer()
