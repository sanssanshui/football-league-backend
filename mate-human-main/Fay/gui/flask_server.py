...
def api_submit():
    ...
    try:
        ...
        # 强制使用通义千问API（覆盖embedding配置）
        if 'key' in existing_config:
            existing_config['key']['gpt_base_url'] = 'https://dashscope.aliyuncs.com/compatible-mode/v1'
            existing_config['key']['gpt_api_key'] = 'sk-c9b8659683a541bfaa8580448ca67766'
            existing_config['key']['embedding_api_model'] = 'text-embedding-v3'
            existing_config['key']['gpt_model_engine'] = 'qwen-plus'
...
def api_get_data():
    ...
    try:
        ...
        # 强制覆盖API配置为通义千问
        return_config = config_util.config.copy()
        if 'key' in return_config:
            return_config['key']['gpt_base_url'] = 'https://dashscope.aliyuncs.com/compatible-mode/v1'
            return_config['key']['gpt_api_key'] = 'sk-c9b8659683a541bfaa8580448ca67766'
            return_config['key']['embedding_api_model'] = 'text-embedding-v3'
            return_config['key']['gpt_model_engine'] = 'qwen-plus'

        return json.dumps({'config': return_config, 'voice_list': voice_list})
    except Exception as e:
