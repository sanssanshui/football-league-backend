...
// Browser-compatible dirname (avoids Node.js 'path' module import)
...
export class LAppLive2DManager {
  ...
  /** Get the first (primary) model for expression/motion control */
  public getModel(): LAppModel | null {
    return this._models.at(0) || null;
  }

  /** Set an expression by name on the primary model */
  public setExpression(name: string): void {
    const model = this.getModel();
    if (model) model.setExpression(name);
  }

  /** Set a random expression on the primary model */
  public setRandomExpression(): void {
    const model = this.getModel();
    if (model) model.setRandomExpression();
  }

  /** Start a specific motion by group + index */
  public startMotion(group: string, no: number, priority: number): void {
    const model = this.getModel();
    if (model) model.startMotion(group, no, priority, this.finishedMotion, this.beganMotion);
  }

  /** Start a random motion from a group */
  public startRandomMotion(group: string, priority: number): void {
    const model = this.getModel();
    if (model) model.startRandomMotion(group, priority, this.finishedMotion, this.beganMotion);
  }
  ...
  public changeCharacter(character: ResourceModel | null) {
    ...
    let dir = dirname(character.link) + "/";
