...
import { ArrowLeft, Clock, MapPin, Repeat2, ShieldCheck, X } from "lucide-react";
...
function EventMarker({ type, minute }: { type: string; minute?: string | number }) {
  ...
  if (type === "goal" || type === "penalty_goal" || type === "own_goal") {
    ...
    return (
      <div className={`relative z-10 flex h-14 w-14 items-center justify-center rounded-full border-4 ${borderColor} bg-white shadow-lg`}>
        <span className="text-xl">⚽️</span>
        {extraLabel && <span className="absolute -top-3 text-[10px] font-black text-white bg-slate-800 px-1.5 py-0.5 rounded">{extraLabel}</span>}
        <span className="absolute -bottom-5 flex items-center justify-center text-[12px] font-black text-slate-700 bg-white/90 px-2 py-0.5 rounded shadow-sm border border-slate-100">{minute}&apos;</span>
      </div>
    );
  }
  if (type === "substitution") {
    return (
      <div className="relative z-10 flex h-14 w-14 items-center justify-center rounded-full border-4 border-emerald-100 bg-white shadow-lg">
  ...
  if (type === "penalty_missed") {
    return (
      <div className="relative z-10 flex h-14 w-14 items-center justify-center rounded-full border-[5px] border-emerald-500 bg-white shadow-lg">
        <span className="absolute -top-3 text-[10px] font-black text-white bg-slate-800 px-1.5 py-0.5 rounded">罚丢</span>
        <span className="text-sm font-black text-slate-700">{minute}&apos;</span>
  ...
  return (
    <div className={`relative z-10 flex h-14 w-14 items-center justify-center rounded-full border-[5px] border-slate-500 bg-white shadow-lg`}>
      <span className="text-sm font-black text-slate-700">{minute}&apos;</span>
    </div>
  );
}
...
function EventCard({ event, align }: { event: MatchEventItem; align: "left" | "right" }) {
  ...
  return (
    <div className={`rounded-2xl bg-slate-50 px-6 py-4 ${align === "left" ? "text-right" : "text-left"}`}>
      <div className="text-xs font-black tracking-[0.18em] text-slate-400">{eventLabel(type)}</div>
      <div className="mt-1 text-lg font-black text-slate-900">{player}</div>
      {detail && <div className="mt-1 text-sm font-bold text-slate-500">{detail}</div>}
    </div>
  );
}
  ...
  return (
    <div className="py-4">
      <div className="flex items-center justify-between mb-2">
        <span className="text-lg font-black text-slate-800 w-16 text-left tabular-nums">{home}</span>
        <span className="text-xs font-bold text-slate-400 tracking-widest uppercase">{label}</span>
        <span className="text-lg font-black text-slate-800 w-16 text-right tabular-nums">{away}</span>
      </div>
      ...
      <div className="relative">
        <div className={`w-9 h-9 md:w-11 md:h-11 rounded-full border-[3px] shadow-lg overflow-hidden bg-white/90 ${borderColor} group-hover:scale-110 transition-transform z-10 relative flex items-center justify-center`}>
          {avatar ? (
            ...
            <span className="text-xs md:text-sm font-black text-slate-700">{number}</span>
          )}
        </div>
        {player.event && (
          <div className="absolute -top-1 -right-3 flex items-center bg-white/90 rounded-full px-1 py-0.5 shadow-sm text-[9px] font-bold z-20 border border-gray-200">
...
export default function MatchDetailPage() {
  ...
  if (!matchDetail) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-100">
      <div className="text-center p-12 bg-white rounded-3xl shadow-xl">
        ...
        <h2 className="text-2xl font-black text-slate-800 mb-4">暂无赛事数据</h2>
        <button onClick={() => router.push(backUrl)} className="bg-emerald-500 text-white px-8 py-3 rounded-xl font-bold">返回列表</button>
      </div>
    </div>
  );
  ...
  return (
    <main className="relative min-h-screen bg-cover bg-center bg-fixed" style={{ backgroundImage: "url('/images/background.jpg')" }}>
      <div className="absolute inset-0 bg-white/75 backdrop-blur-md z-0" />
      <div className="relative z-10 max-w-5xl mx-auto px-6 py-10">
        {/* 返回 + 状态 */}
        <div className="flex items-center justify-between mb-8">
          <button onClick={() => router.push(backUrl)} className="group flex items-center gap-2 text-slate-500 hover:text-emerald-600 bg-white/80 backdrop-blur px-5 py-3 rounded-2xl shadow border border-slate-200 transition-all">
            <ArrowLeft className="w-5 h-5 group-hover:-translate-x-1 transition-transform" />
            <span className="font-bold">返回 {year} 赛季</span>
          </button>
          <div className="flex items-center gap-2 bg-emerald-50 px-5 py-2 rounded-full border border-emerald-100">
            <ShieldCheck className="text-emerald-500 w-4 h-4" />
            <span className="text-emerald-700 font-bold text-sm">{matchDetail.status}</span>
          </div>
        </div>

        {/* 对阵头部 */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white/80 backdrop-blur-2xl rounded-[2.5rem] shadow-xl border border-white/50 p-10 md:p-16 mb-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-10">
            <div className="flex flex-col items-center flex-1">
              ...
              <h2 className="text-3xl font-black text-slate-900">{matchDetail.homeTeam}</h2>
              <p className="text-slate-400 font-bold text-xs tracking-widest mt-1">HOME</p>
            </div>
            <div className="flex flex-col items-center">
              <div className="flex items-center gap-8">
                <span className="text-7xl md:text-8xl font-black text-slate-900 tabular-nums">{regularScore.home}</span>
                <span className="text-4xl font-black text-emerald-400">:</span>
                <span className="text-7xl md:text-8xl font-black text-slate-900 tabular-nums">{regularScore.away}</span>
              </div>
              {hasPenalties && (
                <div className="mt-4 rounded-full bg-slate-100 px-5 py-2 text-xs font-black tracking-[0.24em] text-slate-500 uppercase">
                  点球 [{matchDetail.homePenalties}] : [{matchDetail.awayPenalties}]
                </div>
              )}
              <div className="mt-6 flex items-center gap-4 text-slate-500 text-sm font-bold">
                <span className="flex items-center gap-1"><Clock size={16} className="text-emerald-500" />{matchDetail.datetime}</span>
                <span className="flex items-center gap-1"><MapPin size={16} />{matchDetail.location || '官方主场'}</span>
              </div>
              {referee && <div className="mt-3 text-xs font-black tracking-[0.22em] text-slate-400">裁判 {referee}</div>}
              <div className="mt-3 text-xs font-black uppercase tracking-[0.3em] text-slate-300">{matchDetail.round || `${year}赛季`}</div>
            </div>
            <div className="flex flex-col items-center flex-1">
              <TeamBadge teamName={matchDetail.awayTeam} season={String(year || "2026")} className="w-24 h-24 mb-4" priority />
              <h2 className="text-3xl font-black text-slate-900">{matchDetail.awayTeam}</h2>
              <p className="text-slate-400 font-bold text-xs tracking-widest mt-1">AWAY</p>
            </div>
          </div>
        </motion.div>

        {/* TAB 栏 - 紧贴内容 */}
        <div className="bg-white/80 backdrop-blur-xl rounded-t-[2rem] border border-b-0 border-slate-200 shadow-sm px-4 pt-4 flex gap-2">
          {tabs.map(t => (
            <button key={t.key} onClick={() => setActiveTab(t.key)} className={`flex-1 py-4 rounded-t-2xl font-black text-sm transition-all ${activeTab === t.key ? 'bg-emerald-500 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50'}`}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {/* 内容区 - 紧贴 TAB */}
        <div className="bg-white/80 backdrop-blur-xl rounded-b-[2rem] border border-t-0 border-slate-200 shadow-xl p-8 md:p-12 min-h-[500px]">

          {/* ===== 技术统计 (百度风格) ===== */}
          {activeTab === 'stats' && (
            <div>
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3"><div className="w-3 h-3 rounded-full bg-red-400" /><span className="font-black text-slate-700">{matchDetail.homeTeam}</span></div>
                <h3 className="text-xl font-black text-slate-800">技术统计</h3>
                <div className="flex items-center gap-3"><span className="font-black text-slate-700">{matchDetail.awayTeam}</span><div className="w-3 h-3 rounded-full bg-blue-400" /></div>
              </div>
          ...
          {activeTab === 'events' && (
            <div>
              <h3 className="text-xl font-black text-slate-800 mb-8">赛事事件</h3>
              {events.length > 0 ? (
                <div className="relative pb-8">
                  ...
                  <div className="mt-10 grid grid-cols-2 gap-4 text-sm font-bold text-slate-500 md:grid-cols-4">
                ...
                <div className="flex flex-col items-center justify-center py-24 text-slate-300">
                  ...
                  <p className="text-sm text-slate-400 mt-2">赛事详情爬取后将自动同步至此</p>
                </div>
              )}
            </div>
          )}
          ...
          {activeTab === 'live' && (
            <div>
              <h3 className="text-xl font-black text-slate-800 mb-8">文字直播</h3>
              {textLives.length > 0 ? (
                <div className="space-y-0">
                  {textLives.map((item: TextLiveItem, idx: number) => {
                    ...
                    return (
                    <div key={idx} className="flex items-start gap-6 py-5 border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                      <div className="flex-shrink-0 flex items-center gap-2 pt-1">
                        <div className="w-2 h-2 rounded-full bg-slate-300" />
                        <span className="text-xs text-slate-400 font-bold whitespace-normal max-w-[120px]">{cleanTime}</span>
                      </div>
                      <p className="text-slate-800 font-bold text-base leading-relaxed">{cleanContent}</p>
                    </div>
                  )
                  })}
                </div>
                ...
                <div className="flex flex-col items-center justify-center py-24 text-slate-300">
                  ...
                  <p className="text-sm text-slate-400 mt-2">比赛开始后将实时同步比赛文字播报</p>
                </div>
              )}
            </div>
          )}
          ...
          {activeTab === 'lineup' && (
            <div className="w-full flex flex-col items-center">
              <div className="w-full max-w-5xl flex justify-between items-center mb-4 px-6 py-2 bg-white/80 backdrop-blur-md rounded-full shadow-sm border border-green-100">
                <span className="text-green-800 font-extrabold text-lg tracking-wider">{matchDetail.homeTeam} ({homeFormation})</span>
                <span className="text-gray-400 font-black italic">VS</span>
                <span className="text-green-800 font-extrabold text-lg tracking-wider">{matchDetail.awayTeam} ({awayFormation})</span>
              </div>
