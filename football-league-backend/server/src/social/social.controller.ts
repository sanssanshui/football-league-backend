...
export class SocialController {
  ...
  async followUser(@Request() req: any, @Body('user_id') userId: number) {
    const data = await this.socialService.followUser(
  ...
  async unfollowUser(@Request() req: any, @Query('user_id') userId: number) {
    const data = await this.socialService.unfollowUser(
      Number(req.user.userId),
      Number(userId),
    );
  ...
  async listFollowing(@Request() req: any) {
    const data = await this.socialService.listFollowing(
      Number(req.user.userId),
    );
  ...
  async listFollowers(@Request() req: any) {
    const data = await this.socialService.listFollowers(
      Number(req.user.userId),
    );
    return { code: 200, message: '查询成功', data };
  }
  ...
  async followTeam(@Request() req: any, @Body('team_id') teamId: number) {
    const data = await this.socialService.followTeam(
      Number(req.user.userId),
      Number(teamId),
    );
    return { code: 200, message: '关注成功', data };
  ...
  async unfollowTeam(@Request() req: any, @Query('team_id') teamId: number) {
    const data = await this.socialService.unfollowTeam(
      Number(req.user.userId),
      Number(teamId),
    );
    return { code: 200, message: '取消关注成功', data };
  ...
  async listFollowedTeams(@Request() req: any) {
    const data = await this.socialService.listFollowedTeams(
      Number(req.user.userId),
    );
    return { code: 200, message: '查询成功', data };
  }
  ...
  async sendFriendRequest(
    ...
    const data = await this.socialService.sendFriendRequest(
      Number(req.user.userId),
      Number(receiverId),
    );
  ...
  async listFriendRequests(@Request() req: any) {
    const data = await this.socialService.listFriendRequests(
      Number(req.user.userId),
    );
    return { code: 200, message: '查询成功', data };
  }
  ...
  async acceptFriendRequest(
    ...
    const data = await this.socialService.acceptFriendRequest(
      Number(req.user.userId),
      Number(requestId),
    );
  ...
  async rejectFriendRequest(
    ...
    const data = await this.socialService.rejectFriendRequest(
      Number(req.user.userId),
      Number(requestId),
    );
  ...
  async relationStatus(@Request() req: any, @Query('user_id') userId: number) {
    const data = await this.socialService.getRelationStatus(
      Number(req.user.userId),
      Number(userId),
    );
    return { code: 200, message: '查询成功', data };
