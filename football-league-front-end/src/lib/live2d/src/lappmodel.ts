...
export class LAppModel extends CubismUserModel {
  ...
  public update(): void {
    ...
    if (this._lipsync) {
      ...
      // Check for new audio (with pre-parsed PCM for sync lip-sync)
      let audioItem = Live2dManager.getInstance().playAudio();
      if (audioItem != null) {
        if (audioItem.pcm) {
          // SYNCHRONOUS: PCM pre-parsed — no async delay
          this._wavFileHandler.startWithPcm(
            audioItem.pcm.channels,
            audioItem.pcm.samplesPerChannel,
            audioItem.pcm.sampleRate,
            audioItem.pcm.numChannels
          );
        } else {
          // Legacy async fallback
          this._wavFileHandler.start(audioItem.wavBuffer);
        }
      }

      // Compute RMS for current playback position
      this._wavFileHandler.update(deltaTimeSeconds);
      value = this._wavFileHandler.getRms() * Live2dManager.getInstance().getLipFactor();

      // Force mouth closed when no audio
