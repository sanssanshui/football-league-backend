...
import { Search, User, LogIn, ChevronLeft, ChevronRight, Trophy, Swords, BarChart3, Dumbbell, ShoppingBag } from "lucide-react";
...
const CARDS = [
  ...
  { id: 4, label: '周边商城', icon: ShoppingBag, color: '#f59e0b' },
];
              ...
              return (
                ...
                >
                  ...
                  <div className="px-8 py-6 h-[380px] overflow-hidden">
                    ...
                    {card.id === 4 && <ShopCardBody router={router} />}
                  </div>
                </motion.div>
              );
            })}
...
function ShopCardBody({ router }: { router: ReturnType<typeof useRouter> }) {
  return (
    <div className="flex flex-col items-center justify-center h-full gap-5">
      <div className="flex gap-3">
        {['球衣', '围巾', '帽子', '足球', '钥匙扣'].map((item, i) => (
          <div key={i} className="w-14 h-14 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-xs text-amber-400 font-medium">
            {item}
          </div>
        ))}
      </div>
      <div className="text-center">
        <h3 className="text-white text-xl font-bold mb-1">周边商城</h3>
        <p className="text-white/50 text-sm">官方授权周边，球衣、围巾、纪念品一站购齐</p>
        <p className="text-white/30 text-xs mt-1">支持支付宝支付 · 正品保障</p>
      </div>
      <button
        onClick={() => router.push('/shop')}
        className="w-full py-3 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 text-white text-sm font-semibold hover:shadow-[0_0_20px_rgba(245,158,11,0.5)] transition-all"
      >
        🛍️ 进入商城
      </button>
    </div>
