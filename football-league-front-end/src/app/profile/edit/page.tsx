...
export default function EditProfilePage() {
  ...
  return (
    <div className="min-h-screen flex justify-center items-center font-sans overflow-hidden relative" onMouseMove={onMouseMove}>
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ type: "spring", stiffness: 100, damping: 15 }}
        className="relative z-10 w-[560px] bg-black/40 backdrop-blur-2xl border border-white/10 rounded-[32px] p-10 shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
        ...
        <div className="flex flex-col items-center mb-8">
          <div className="relative cursor-pointer group" onClick={() => fileRef.current?.click()}>
            <div className="w-24 h-24 rounded-full border-2 border-[#008000] overflow-hidden bg-[#1a1a1a] flex items-center justify-center shadow-[0_0_20px_rgba(0,128,0,0.3)]">
              {preview
        ...
        <div className="space-y-5">
            ...
            <div className="flex gap-2">
              {["男", "女", "保密"].map(g => (
                <button key={g} onClick={() => setForm(p => ({ ...p, gender: g }))}
                  className={`flex-1 py-2 rounded-xl text-sm font-medium border transition-all ${form.gender === g ? "bg-[#008000]/80 border-[#008000] text-white shadow-[0_0_12px_rgba(0,128,0,0.3)]" : "bg-white/5 border-white/10 text-white/60 hover:bg-white/10 hover:text-white"}`}>
                  {g}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <input type="date" value={form.birthday}
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm outline-none focus:border-[#008000]/60 focus:bg-white/10 transition-all [color-scheme:dark]" />
          </div>
          ...
          <div>
            ...
            <input type="text" value={form.birthplace} placeholder="例：江苏省南京市"
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm outline-none focus:border-[#008000]/60 focus:bg-white/10 transition-all placeholder:text-white/20" />
          </div>
          ...
          <div>
            ...
            <textarea value={form.bio} placeholder="写点什么介绍自己..." rows={3}
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm outline-none focus:border-[#008000]/60 focus:bg-white/10 transition-all placeholder:text-white/20 resize-none" />
          </div>
        </div>
        ...
        <div className="flex gap-3 mt-8">
          <button onClick={() => router.back()}
            className="flex-1 py-3 rounded-xl bg-white/5 border border-white/10 text-white/60 text-sm hover:bg-white/10 transition-all">
            取消
          </button>
